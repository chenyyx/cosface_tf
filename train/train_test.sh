...
CMD="python train_multi_gpu_test.py --logs_base_dir logs/cosface_yyx/multi_gpu/ --models_base_dir models/cosface_yyx/multi_gpu/ --data_dir /home/face/chenyyx/cosface_tf/dataset/CASIA-WebFace-112X96 --list_file /home/face/chenyyx/cosface_tf/dataset/CASIA-WebFace-112X96_list.txt --model_def models.inception_resnet_v1  --optimizer ${OPT} --learning_rate -1 --max_nrof_epochs 20 --random_flip --learning_rate_schedule_file ${LR_FILE}  --num_gpus ${NUM_GPUS} --weight_decay ${WEIGHT_DECAY} --loss_type ${LOSS_TYPE} --scale ${SCALE} --weight ${WEIGHT} --alpha ${ALPHA} --network ${NETWORK} ${FC_BN} --image_height ${IMAGE_HEIGHT} --image_width  ${IMAGE_WIDTH} --embedding_size ${EMBEDDING_SIZE} 2>&1 | tee -a logs/cosface_yyx/multi_gpu/cosface_mg_20180920_2_inception.log"
echo Run "$ARGS ${CMD}"
eval "$ARGS ${CMD}"