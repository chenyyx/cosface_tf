...
CMD="python train/train_multi_gpu.py --logs_base_dir logs/cosface_yyx/multi_gpu/ --models_base_dir models/cosface_yyx/multi_gpu/ --data_dir /Users/chenyao/Documents/dataset/112X96/CASIA-WebFace/CASIA-WebFace-112X96 --list_file /Users/chenyao/Documents/dataset/112X96/CASIA-WebFace/CASIA-WebFace-112X96_list.txt --model_def models.sphere_network  --optimizer ${OPT} --learning_rate -1 --max_nrof_epochs 20 --random_flip --learning_rate_schedule_file ${LR_FILE}  --num_gpus ${NUM_GPUS} --weight_decay ${WEIGHT_DECAY} --loss_type ${LOSS_TYPE} --scale ${SCALE} --weight ${WEIGHT} --alpha ${ALPHA} --network ${NETWORK} ${FC_BN} --image_height ${IMAGE_HEIGHT} --image_width  ${IMAGE_WIDTH} --embedding_size ${EMBEDDING_SIZE}"
echo Run "$ARGS ${CMD}"
eval "$ARGS ${CMD}"
